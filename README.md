...
### RSVP
The RSVP section now embeds your Google Form directly (with an "Open Form in
New Tab" fallback link below it, in case a guest's browser blocks the embed).
If you ever need to swap in a different form, search `index.html` for
`docs.google.com/forms` — it appears twice (the iframe `src` and the
fallback link's `href`) and both need updating together.
